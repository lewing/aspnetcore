...
namespace Microsoft.AspNetCore.Blazor.DevServer.Server
{
    internal class Startup
    {
        ...
        {
            services.AddRouting();
            services.AddBlazorHosting();
            services.AddOptions<BlazorHostingOptions>().Configure<IWebHostEnvironment>((options, environment) =>
            {
                options.ClientAssemblyPaths.Add(FindClientAssemblyPath(environment));
            });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment environment, IConfiguration configuration)
        {
            ...
            {
                endpoints.MapFallbackToClientSideBlazor(FindClientAssemblyPath(environment));
            });
        }
